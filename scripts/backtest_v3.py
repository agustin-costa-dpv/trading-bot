...
Configuración final guiada por datos (v2 → v3):

DECISIONES (basadas en backtest v2):
1. MEAN_REVERSION: desactivada globalmente (PF 0.84 consistente)
2. ARBITRAJE: SOLO en ETH (PF 1.67). Desactivada en BTC y SOL.
3. TREND_FOLLOWING: solo BTC y ETH (SOL descartado)
4. SOL: fuera del universo

Uso:
    python -m scripts.backtest_v3 --meses 6
"""
...
SIMBOLOS = {"BTC": "BTCUSDT", "ETH": "ETHUSDT"}  # SOL fuera

# ────────────────────────────────────────────
# MATRIZ DE ESTRATEGIAS HABILITADAS POR ACTIVO
# ────────────────────────────────────────────

ESTRATEGIAS_HABILITADAS = {
    "BTC": {"TREND_FOLLOWING"},
    "ETH": {"TREND_FOLLOWING", "ARBITRAJE"},
    # SOL: ninguna (fuera del universo)
}
...
# Params por estrategia
PARAMS_ESTRATEGIA = {
    "MEAN_REVERSION":  {"tp": 1.0, "sl": 0.8, "horizonte": 3},
    "TREND_FOLLOWING": {"tp": 2.0, "sl": 1.0, "horizonte": 6},
    "ARBITRAJE":       {"tp": 1.5, "sl": 0.7, "horizonte": 2},
}
...
def descargar_velas_historicas(simbolo: str, meses: int, intervalo: str) -> pd.DataFrame:
...
def evaluar(row, activo: str) -> Optional[dict]:
    """
    NUEVO: filtra por activo + estrategia habilitada.
    """
    estrategias_ok = ESTRATEGIAS_HABILITADAS.get(activo, set())
    ...
    if regimen == "MOVIMIENTO_BRUSCO":
        ...
        # MEAN_REVERSION desactivada globalmente
        return None
...
def simular_trade(df: pd.DataFrame, idx: int, senal: dict, activo: str) -> Optional[Trade]:
    ...
    for i in range(1, horizonte + 1):
        ...
        if direccion == "SUBE":
            if low <= sl:
                return _trade(df, idx, idx+i, senal, activo, precio_entrada, sl, -sl_pct, "SL")
            if high >= tp:
                return _trade(df, idx, idx+i, senal, activo, precio_entrada, tp, tp_pct, "TP")
        else:
            if high >= sl:
                return _trade(df, idx, idx+i, senal, activo, precio_entrada, sl, -sl_pct, "SL")
            if low <= tp:
    ...
    return _trade(df, idx, idx + horizonte, senal, activo,
                   precio_entrada, precio_salida, pnl, "TIMEOUT")


def _trade(df, i_in, i_out, senal, activo, p_in, p_out, pnl, resultado):
    return Trade(
        activo=activo,
        entrada_timestamp=df.index[i_in],
        salida_timestamp=df.index[i_out],
        direccion=senal["direccion"],
...
def calcular_metricas(trades: list[Trade]) -> dict:
    ...
    p_ = [p for p in pnls if p < 0]
    total = len(trades)
    wr = len(g) / total * 100
    pf = sum(g) / abs(sum(p_)) if p_ else float("inf")
    s = pd.Series(pnls)
    ...
    return {
        ...
        "avg_ganador": sum(g)/len(g) if g else 0,
        "avg_perdedor": sum(p_)/len(p_) if p_ else 0,
...
def metricas_por_estrategia(trades: list[Trade]) -> dict:
    return {est: calcular_metricas([t for t in trades if t.estrategia == est])
            for est in ["TREND_FOLLOWING", "ARBITRAJE"]}


def imprimir_reporte(activo: str, trades: list[Trade], regimen_counts: dict):
    print(f"\n{'=' * 60}")
    print(f"  RESULTADOS: {activo}")
    print(f"  Estrategias habilitadas: {', '.join(ESTRATEGIAS_HABILITADAS.get(activo, ['ninguna']))}")
    print(f"{'=' * 60}")
...
def main():
    ...
    activos = list(SIMBOLOS.keys())

    print(f"\n{'#' * 60}")
    print(f"#  BACKTEST v3 FINAL — {args.meses} meses")
    print(f"#  Configuración:")
    print(f"#    BTC: TREND_FOLLOWING")
    print(f"#    ETH: TREND_FOLLOWING + ARBITRAJE")
    print(f"#    SOL: desactivado")
    print(f"#    MEAN_REVERSION: desactivada globalmente")
    print(f"{'#' * 60}")
    ...
    if todos_trades:
        ...
        } for t in todos_trades]).to_csv("backtest_v3_trades.csv", index=False)
        print(f"\n💾 backtest_v3_trades.csv guardado")
